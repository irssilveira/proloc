...
$(document).ready(function () {
    ...
    $(".verInfo").click(function (e) {
        ...
        $('#resultadoContato').empty();
        var html = '';
        $.get('/clientes/informacao-cliente?id='+id).success(function (data,status) {
            var cliente = data;

            var html = '';
            if(cliente.contato.length > 0){
                for(var i = 0;i<cliente.contato.length;i++){

                    html+= '<tr><td class="vertical-middle" data-title="Nome">'+cliente.contato[i].contato+'</td><td data-title="Email">'+cliente.contato[i].email+'</td> <td data-title="Telefone">('+cliente.contato[i].operadora+') '+cliente.contato[i].telefone+'</td>';
                    html+='</tr>';
                }

            }else{
                html+='<span class="text-center label label-danger text-uppercase">Não há telefone cadastrado</span> <br /><br />';
                html+='<a class="btn btn-block btn-primary" target="_blank" href="clientes/'+cliente.id+'/editarCliente">Cadastrar Telefone</a>';
            }
            $('#resultadoContato').append(html);
        });

       $('#informacaoCliente').modal('show');

    })

    $('#franqueadoCheck').on('click',function() {
        if($(this).is(':checked')){
            $('#temFranqueado').removeClass('hide');
        }else {
            $('#temFranqueado').addClass('hide');
        }
    });
    //js aba frete
    ...
    $('.confirmacaoLink').click(function () {
        if(confirm('Tem certeza?')){
            location.href="/frete/novo";
        }
    ...
    $(document).on('click', 'input[type=radio]', function(){
        ...
        if(escolhido == "juridica"){
            ...
            $('#nomeFantasia').addClass('show').removeClass('hide').val('').prop('disabled',false);
            ...
            $('#nomeFantasia').removeClass('show').addClass('hide').val('').prop('disabled',true);
    ...
    $('#cep_correspondencia').blur(function(){
        /* Configura a requisição AJAX */
        console.log('teste');
        $.ajax({
            url : '/consultar_cep', /* URL que será chamada */
            type : 'GET', /* Tipo da requisição */
            data: 'cep=' + $('#cep_correspondencia').val(), /* dado que será enviado via POST */
            dataType: 'json', /* Tipo de transmissão */
            success: function(data){
                console.log(data.sucesso);
                if(data.sucesso == 1){
                    $('#endereco_correspondencia').val(data.rua);
                    $('#bairro_correspondencia').val(data.bairro);
                    $('#cidade_correspondencia').val(data.cidade);
                    $('#estado_correspondencia').val(data.estado);
                    $('#referencia_correspondencia').focus();
    ...
    $('#leadsAtivos').on('click',function (e) {
        e.preventDefault();
        $.ajax({
            type: 'GET',
            url: '/leads/leadstodos',
            dataType: 'json',
            data: 'leads',
            beforeSend:function(){
            ...
            success: function (data) {
                ...
                for(var i = 0;i<tamanho;i++){
                    ...
                    html+='  <td data-title="Retorno" style="background-color:#5bf98b">'+novaData+'</td>';
